...
class NoteInput extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      body: '',
      // State tambahan untuk pesan error [Advanced]
      errorMessage: ''
    };
  ...
  onTitleChangeEventHandler(event) {
    ...
    // TODO [Skilled] batasi judul maksimal 50 karakter.
    if (titleInput.length <= limit) {
      this.setState({
        title: titleInput,
      });
    }
  ...
  onBodyChangeEventHandler(event) {
    ...
    this.setState({
      ...
      errorMessage: '' // Reset error saat user mulai mengetik ulang
    });
  }
    ...
    return (
      <div className="note-input" data-testid="note-input">
        ...
        {this.state.errorMessage && (
          <p className="note-input__feedback--error" style={{ color: 'red', marginBottom: '10px' }}>
            {this.state.errorMessage}
          </p>
        )}
        ...
        >
          {/* TODO [Skilled] tampilkan sisa karakter secara dinamis */}
          <p
            className={`note-input__title__char-limit ${remainingChars < 10 ? 'warning' : ''}`}
